	...
	private void showPositions(Sun[] theSunArray, java.awt.Graphics graphics) {
		for (int i = 0; i < theSunArray.length; i++) {
			...
			if (x < 0 || x > 1920 || y < 0 || y > 1080){
				...
				if(max >7){
						try {
							bufferedImage = (new Robot()).createScreenCapture(graphics.getClipBounds());
						} catch (AWTException e2) {
							// TODO Auto-generated catch block
							e2.printStackTrace();
						}
						try {
						ImageIO.write(bufferedImage, "png", new File("C:/Temp/test"+Calendar.getInstance().getTimeInMillis()+".png"));
					} catch (Exception e1) {
					}
				sunArray[0] = null;
